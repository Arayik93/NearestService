...
public class BeautySalon extends RealmObject implements Service {
    ...
    private String category = "beautySalon";

    public BeautySalon( String name, String description, String address, double latitude, double longitude) {
        this.name = name;
    ...
    public void saveInDatabase(String name, String description, String address, double latitude, double longitude) {
        BeautySalon service = new BeautySalon(name, description, address, latitude, longitude);
        Realm realm = Realm.getDefaultInstance();
        realm.beginTransaction();
        RealmResults rel = realm.where(BeautySalon.class).findAll();
        int id = 1;
        if (rel.size() != 0) {
            BeautySalon serviceFromDB = (BeautySalon) rel.last();
    ...
    public UniversalService showYourFullInfo() {
        return new UniversalService(this.name,this.description, this.address, this.category, this.rating,
                this.latitude, this.longitude, R.drawable.markerbeautysalon, this.id);
    }
}
